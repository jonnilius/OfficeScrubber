...
echo.
echo %_ln%
echo Suche Office Versionen, bitte warten . . .
...
if %winbuild% GEQ 7600 (
echo.
echo. --- Office 2016 und später ---
echo [C] Bereinige vNext Lizenzen %dONXT%
echo [R] Entferne alle Lizenzen
echo [T] Zurücksetzen von C2R Lizenzen
echo [U] Deinstalliere alle Schlüssel
)
echo.
echo %_ln%
if %_wxp% equ 0 (
choice /c 12345678CRTU0 /n /m "Wählen eine Option oder Drücken 0 zum Beenden: "
set _er=!ERRORLEVEL!
) else (
set /p _pt="Geben Sie eine Menüoption ein und drücken Sie die Eingabetaste, oder 0 zum Beenden: "
)
if defined _pt (
...
echo -------
echo Hinweis:
echo Manuelles Auswählen aller Optionen ist nicht erforderlich und dauert zu lange.
echo.
echo %_ln%
if %_wxp% equ 0 (
choice /c 123456780 /n /m "Ändern Sie die Menüoptionen oder drücken Sie 0 zum Beenden: "
set _er=!ERRORLEVEL!
) else (
set /p _pt="Geben Sie eine Menüoption ein und drücken Sie die Eingabetaste, oder 0 zum Beenden: "
)
...
call :Hdr
echo.
echo Deinstalliere Produktschlüssel . . .
call :cKMS %_Nul3%
if %winbuild% GEQ 7600 (
...
call :Hdr
if %_pwsh% EQU 0 (
set "msg=FEHLER: Windows Powershell nicht gefunden."
goto :TheEnd
)
call :rOUWP
set "msg=Fertig."
...
if exist "!_file!" (
echo.
echo Führe OfficeClickToRun.exe aus . . .
%_Nul3% call :CloseC2R
...
:rOUWP
echo.
echo Entferne Office UWP Anwendungen . . .
...
echo.
echo Aktualisiere Windows Insider Preview Lizenzen . . .
...
set "msg=Erledigt. Es wird empfohlen den Computer neuzustarten."
goto :TheEnd
...
call :Hdr
echo.
echo Zurücksetzen von Office C2R Lizenzen . . .
if %_O16CTR% equ 0 (
set "msg=FEHLER: Kein installiertes Office ClickToRun gefunden."
goto :TheEnd
)
set "_InstallRoot="
...
set "msg=FEHLER: Konnte Office Licenses Integrator.exe nicht finden"
goto :TheEnd
)
...
set "msg=FEHLER: Es konnten keine Original installiertes Office gefunden werden."
goto :TheEnd
)
call :cSPP
echo.
echo Installiere Office C2R Lizenzen . . .
for %%a in (%_SKUs%) do (
...
echo Zum Bekommen, Rechts-Klick auf dieses Skript und wähle "Als Administrator auführen" aus
goto :E_Exit

:E_VBS
echo %_err%
echo VBScript engine ist nicht installiert.
echo Dieses Skript benötigt es zum Funktionieren
goto :E_Exit

:E_WSH
echo %_err%
echo Windows Script Host ist deaktiviert.
echo Dieses Skript benötigt es zum Funktionieren
goto :E_Exit
...
echo.
echo Beliebige Taste drücken zum Beenden.
pause >nul
goto :eof

----- Anfang vom wsf Skript --->
<package>
   <job id="WmiQuery">
